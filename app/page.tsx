...
export default function CinematicHome() {
  ...
  // --- NEW: CHỐNG LỖI IOS SAFARI ---
  const [animKey, setAnimKey] = useState(0); // Để reset animation khi user quay lại tab
  ...
  // --- FIX: CHUÔNG BÁO THỨC SAFARI & AUTO REFRESH ---
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        ...
        // Nếu vắng mặt quá 1 tiếng -> Refresh trang hoàn toàn cho sạch memory
        if (hoursPassed > 1) {
          window.location.reload();
          return;
        }

        // Nếu vắng mặt ít -> Chỉ cập nhật data và khởi động lại chữ chạy
        fetchSchedules();
        fetchAnnouncements();
  ...
  // CSS CLASSES - UPDATE OVERFLOW-HIDDEN THEO GỢI Ý
  ...
  return (
    // FIX 1: Dùng h-[100dvh] cho viewport động iOS và 'isolate' để chặn Safari gộp layer bừa bãi
    <div className="relative h-[100dvh] bg-slate-900 font-sans text-slate-100 overflow-hidden flex flex-col isolate">
        ...
        {/* MARQUEE - FIX TRIỆT ĐỂ: Bỏ Blur ở đây vì nó gây xung đột layer trên iPhone */}
        {marqueeList.length > 0 && (
            <div className="sticky top-0 z-[60] w-full bg-black border-b border-white/10 shadow-lg pt-[env(safe-area-inset-top)]">
                     ...
                     <div className="h-full flex flex-col justify-end items-start gap-2">
                        {status.type === 'happening' && status.item && (
                            <div className={`${cardStyle} border-red-500/30`}>
                                ...
                                <div className="inline-flex items-center gap-1.5 text-white/80 font-medium mb-3 bg-black/20 px-2 py-0.5 sm:px-3 sm:py-1 rounded-lg border border-white/5 backdrop-blur-sm"><MapPin size={14} className="text-gold"/><span className="text-xs sm:text-sm uppercase tracking-wide">{status.item.location}</span></div>
                                <div className="flex items-center gap-2 text-white/80 font-serif italic text-base sm:text-lg border-b border-white/10 pb-3 sm:pb-4 mb-0"><User size={18}/> <span className="truncate">Chủ tế: {status.item.priest_name}</span></div>
                                <div className="mt-4 flex items-center gap-3 text-red-400 font-bold uppercase tracking-widest animate-pulse"><div className="w-2 h-2 bg-red-500 rounded-full"></div>Thánh lễ đang cử hành</div>
                            </div>
                        )}
                        {status.type === 'countdown' && status.item && (
                            <div className={`${cardStyle} border-gold/30`}>
                                ...
                                <div className="flex items-center gap-2 mb-3 sm:mb-4 text-white/90"><MapPin size={14} className="text-gold"/><span className="text-xs sm:text-sm font-bold uppercase tracking-widest">{status.item.location}</span></div>
                                <div className="bg-black/30 rounded-xl p-3 sm:p-4 border border-white/10 mb-1 inline-block w-full text-center lg:text-left"><div className="text-[10px] text-white/60 uppercase tracking-widest mb-1 font-bold">Thời gian còn lại</div><div className="font-mono text-4xl sm:text-5xl lg:text-6xl font-bold text-white tabular-nums drop-shadow-2xl tracking-tighter">{status.diffString}</div></div>
                            </div>
                        )}
                        {status.type === 'upcoming' && status.item && (
                            <div className={cardStyle}>
                                <div className="flex items-center gap-2 mb-2 sm:mb-3"><div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${status.isTomorrow ? 'bg-blue-400 shadow-[0_0_10px_#60a5fa]' : 'bg-green-400 shadow-[0_0_10px_#4ade80]'}`}></div><span className={`text-[10px] sm:text-xs font-bold uppercase tracking-widest ${status.isTomorrow ? 'text-blue-300' : 'text-green-300'}`}>{status.isTomorrow ? 'Lễ ngày mai' : 'Sẵn sàng'}</span></div>
                                <div className="mb-3 sm:mb-4"><div className="text-gold-light text-[10px] sm:text-xs uppercase tracking-[0.2em] mb-4 font-bold">Thánh Lễ Kế Tiếp</div><h1 className="font-serif font-bold text-3xl sm:text-4xl lg:text-6xl text-white text-shadow leading-tight">{status.item.title}</h1></div>
                                <div className="flex gap-4 sm:gap-6 border-t border-white/20 pt-3 sm:pt-4"><div><div className="text-[10px] text-white/50 uppercase mb-0.5 font-bold">Thời gian</div><div className="text-xl sm:text-2xl font-bold text-white font-mono">{status.item.start_time.slice(0,5)}{status.isTomorrow && <span className="text-xs text-white/50 ml-1 align-top font-sans font-medium">(Ngày mai)</span>}</div></div><div className="w-px bg-white/20"></div><div><div className="text-[10px] text-white/50 uppercase mb-0.5 font-bold">Địa điểm</div><div className="text-xl sm:text-2xl font-serif italic text-white">{status.item.location}</div></div></div>
                            </div>
                        )}
                        {status.type === 'finished' && (
                            <div className={cardStyle}><h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2 text-shadow">Lạy Mẹ La Vang</h1><p className="text-base sm:text-lg text-white/80 italic font-serif">Xin cầu cho chúng con.</p></div>
                        )}
                        <div className={widgetContainerStyle}>
                            <div className={widgetStyle}>{weather.code >= 51 ? <CloudRain className="text-blue-400 w-6 h-6 sm:w-8 sm:h-8"/> : <Sun className="text-yellow-400 w-6 h-6 sm:w-8 sm:h-8"/>}<div><div className="text-xl sm:text-2xl font-bold text-white font-mono">{weather.temp}°C</div><div className="text-[10px] sm:text-xs text-white/70 uppercase font-bold">{weather.desc}</div></div></div>
                            <div className={widgetStyle}><Clock className="text-white/60 w-5 h-5 sm:w-6 sm:h-6"/><div className="text-2xl sm:text-3xl font-bold font-mono text-white tracking-widest">{format(now, 'HH:mm')}</div></div>
                        </div>
                     </div>
                </div>
                    ...
                    <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl sm:rounded-3xl flex flex-col overflow-hidden h-[500px] sm:h-[400px] lg:h-full shadow-2xl">
                        <div className="p-5 sm:p-6 border-b border-white/10 bg-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 flex-none">
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                <button onClick={() => setShowWeekModal(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gold/20 hover:bg-gold/40 border border-gold/30 py-3 sm:py-1.5 px-4 sm:px-3 rounded-full transition group cursor-pointer active:scale-95"><Calendar size={16} className="text-gold sm:w-3.5 sm:h-3.5"/><span className="text-sm sm:text-xs font-bold text-gold uppercase tracking-wider">Lịch Tuần</span></button>
                                <span className="flex-1 sm:flex-none text-center text-sm sm:text-xs font-bold text-white/80 bg-white/10 py-3 sm:py-1.5 px-4 sm:px-3 rounded-full backdrop-blur border border-white/10 font-mono">{format(now, 'dd/MM/yyyy', {locale: vi})}</span>
                            </div>
                        </div>
                        <div className="overflow-y-auto flex-grow p-3 sm:p-4 custom-scrollbar">
                            <div className="space-y-3 sm:space-y-6 relative pl-6 sm:pl-8 before:absolute before:left-[27px] sm:before:left-[35px] before:top-4 before:bottom-4 before:w-[1px] before:bg-white/20">
                                ...
                                schedules.map(ev => {
                                    ...
                                    if (isHappening) rowClass += "bg-gradient-to-r from-red-600/20 to-transparent border-l-4 border-l-red-500 shadow-xl transform scale-[1.03]";
                                    else if (isUpcoming) rowClass += (status.item?.id === ev.id ? "bg-white/5 border-l-4 border-l-gold/50" : "hover:bg-white/10");
                                    else rowClass += "opacity-40 grayscale hover:opacity-100 hover:grayscale-0";
                                    return (
                                        <div key={ev.id} className={rowClass}>
                                            <div className={`w-16 sm:w-24 text-right text-2xl sm:text-4xl font-mono tracking-tight shrink-0 ${isHappening ? 'text-red-400 font-bold' : (isUpcoming ? 'text-gold font-bold' : 'text-white')}`}>{ev.start_time.slice(0,5)}</div>
                                            <div className="flex-grow min-w-0 pl-2 sm:pl-2">
                                                <div className={`text-lg sm:text-2xl font-serif leading-tight mb-1 ${isHappening ? 'text-white font-bold' : (isUpcoming ? 'text-gold-light font-bold' : 'text-white/90')}`}>{ev.title}</div>
                                                <div className="flex items-start gap-1.5 text-xs sm:text-base text-white/70 uppercase font-bold tracking-wider"><MapPin size={12} className="shrink-0 mt-0.5 sm:mt-1"/><span className="leading-snug">{ev.location}</span></div>
            ...
            <div className={`fixed inset-0 z-[200] flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md transition-opacity duration-300 ${modalVisible ? 'opacity-100' : 'opacity-0'}`}>
                <div className={`bg-slate-900/90 backdrop-blur-xl border border-white/20 rounded-2xl sm:rounded-3xl w-[95%] sm:w-full max-w-6xl max-h-[85vh] sm:max-h-[90vh] flex flex-col shadow-2xl transition-transform duration-300 ${modalVisible ? 'scale-100' : 'scale-95'}`}>
                    <div className="flex justify-between items-center p-4 sm:p-6 border-b border-white/10 bg-white/5 rounded-t-2xl sm:rounded-t-3xl">
                        <div><h2 className="font-serif text-xl sm:text-3xl font-bold text-white text-shadow">Lịch Thánh Lễ trong 7 ngày tới</h2><p className="text-white/60 text-xs sm:text-sm mt-1">Chi tiết các Thánh Lễ</p></div>
                        <button onClick={() => setShowWeekModal(false)} className="p-2 bg-white/10 hover:bg-red-600/80 rounded-full transition group active:scale-90"><X className="text-white" size={24}/></button>
                    </div>
                                ...
                                return Object.entries(grouped).map(([date, items]: any) => {
                                    ...
                                    return (
                                        <div key={date} className={`rounded-xl border p-3 sm:p-4 transition duration-300 ${isTodayDate ? 'border-gold bg-gold/10 ring-1 ring-gold/50' : 'border-white/10 bg-white/5 hover:bg-white/10'}`}>
                                            <div className="flex justify-between items-center mb-2 sm:mb-3 border-b border-white/10 pb-2"><span className="font-serif font-bold text-lg sm:text-xl text-white capitalize">{format(parseISO(date), 'EEEE', {locale: vi})}</span><span className={`text-[10px] sm:text-xs font-bold border border-current px-2 py-0.5 rounded-full font-mono ${isTodayDate ? 'text-gold' : 'text-white/70'}`}>{format(parseISO(date), 'dd/MM')}</span></div>
                                            <div className="space-y-2">
                                                ...
                                                items.map((ev: any) => (
                                                    <div key={ev.id} className="flex items-start gap-2 sm:gap-3 py-2 border-b border-white/5 last:border-0 group">
                                                        ...
                                                        <div className="flex-grow min-w-0"><div className="text-xs sm:text-sm font-bold text-white leading-tight break-words whitespace-normal">{ev.title}</div><div className="text-[10px] sm:text-[11px] text-white/50 uppercase flex items-start gap-1 mt-1"><MapPin size={10} className="shrink-0 mt-0.5"/><span className="break-words whitespace-normal">{ev.location}</span></div>{ev.priest_name && <div className="text-[10px] sm:text-[11px] text-white/40 italic mt-0.5 break-words whitespace-normal flex items-start gap-1"><User size={10} className="shrink-0 mt-0.5"/><span>{ev.priest_name}</span></div>}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
