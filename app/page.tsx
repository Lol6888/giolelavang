...
export default function CinematicHome() {
  ...
  // --- CHIẾN THUẬT HARDCORE V2: RELOAD TỨC THÌ ---
  const isInitialMount = useRef(true);

  useEffect(() => {
    const handleVisibilityChange = () => {
      // KIỂM TRA: Nếu là iPhone/iPad VÀ quay lại tab sau khi đã rời đi
      const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
      
      if (document.visibilityState === 'visible' && isIOS) {
        // Ép trình duyệt xóa sạch bộ nhớ đệm và tải lại trang từ đầu
        // Điều này đảm bảo trang luôn ở trạng thái "vừa load xong" 100%
  ...
  const getStatus = () => {
    ...
    if(next) {
        ...
        if (diffMinutes <= COUNTDOWN_THRESHOLD_MINUTES) {
            ...
            return { type: 'countdown', item: next, diffString: `${Math.floor(diffSec/60).toString().padStart(2,'0')}:${(diffSec%60).toString().padStart(2,'0')}` };
        }
                     ...
                     <div className="h-full flex flex-col justify-end items-start gap-2">
                        {status.type === 'happening' && status.item && (
                            <div className={`${cardStyle} border-red-500/30`}>
                                <div className="flex items-center gap-2 sm:gap-3 mb-2"><span className="relative flex h-3 w-3"><span className="animate-ping absolute h-full w-full rounded-full bg-red-500 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-red-600"></span></span><span className="text-red-400 text-xs font-bold uppercase tracking-widest">Đang diễn ra</span></div>
                        ...
                        {status.type === 'countdown' && status.item && (
                            <div className={`${cardStyle} border-gold/30`}>
                                ...
                                <div className="bg-black/30 rounded-xl p-4 border border-white/10 mb-1 w-full text-center lg:text-left"><div className="text-[10px] text-white/60 uppercase mb-1 font-bold">Thời gian còn lại</div><div className="font-mono text-4xl sm:text-5xl lg:text-6xl font-bold text-white tabular-nums tracking-tighter">{status.diffString}</div></div>
                            </div>
                        )}
                    ...
                    <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl sm:rounded-3xl flex flex-col overflow-hidden h-[500px] sm:h-[400px] lg:h-full shadow-2xl isolate">
                        <div className="p-5 sm:p-6 border-b border-white/10 bg-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 flex-none">
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                <button onClick={() => setShowWeekModal(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gold/20 hover:bg-gold/40 border border-gold/30 py-2 px-4 rounded-full active:scale-95"><Calendar size={16} className="text-gold"/><span className="text-xs font-bold text-gold uppercase tracking-wider">Lịch Tuần</span></button>
                        ...
                        <div className="overflow-y-auto flex-grow p-4 custom-scrollbar">
                            <div className="space-y-4 relative pl-8 before:absolute before:left-[35px] before:top-4 before:bottom-4 before:w-[1px] before:bg-white/20">
                                ...
                                schedules.map(ev => {
                                    ...
                                    const isHappening = now.getTime() >= start && now.getTime() < start + 1800000;
            ...
            <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md transition-opacity duration-300 ${modalVisible ? 'opacity-100' : 'opacity-0'} isolate`}>
                <div className={`bg-slate-900/90 border border-white/20 rounded-3xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl transition-transform duration-300 ${modalVisible ? 'scale-100' : 'scale-95'}`}>
                    <div className="flex justify-between items-center p-6 border-b border-white/10 bg-white/5 rounded-t-3xl">
                        ...
                        <button onClick={() => setShowWeekModal(false)} className="p-2 bg-white/10 rounded-full hover:bg-red-600/80 transition active:scale-90"><X className="text-white" size={24}/></button>
                    </div>
